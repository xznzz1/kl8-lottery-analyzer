...
LOTTERY_ROOT = PROJECT_ROOT.parent
...
def _child_environment():
    ...
    pycache_root = PROJECT_ROOT / "data_cache" / "pycache"
    for path in (temp_root, cache_root, cache_root / "matplotlib", pycache_root):
...
def create_parser():
    ...
    parser.add_argument(
        "--cal_nums_list", default="4,5,7,10", type=str, help="cal_nums_list"
    )
    parser.add_argument(
        "--total_create_list", default="50,100,1000", type=str, help="total_create_list"
    )
    parser.add_argument(
        "--nums_range", default="2023140,2023241", type=str, help="nums_range"
    )
    ...
    parser.add_argument(
        "--download", default=1, type=int, help="download data before processing"
    )
    return parser
...
def _main(
    _total_create, _cal_nums, _current_nums, _process="./kl8_analysis.py", args=None
):
    ...
    try:
        result = subprocess.run(
            [
                ...
                "5",
                "--path",
                str(_total_create) + "_" + str(abs(int(_cal_nums))),
                "--repeat",
                str(args.repeat),
                "--simple_mode",
                "1",
                "--random_mode",
                str(args.random_mode),
                "--max_workers",
...
def main():
        ...
        for _total_create in total_create_list:
            for _cal_nums in cal_nums_list:
                for _current_nums in range(begin, end + 1):
                    tasks.append(
                        (_total_create, _cal_nums, _current_nums, kl8_analysis, args)
                    )
    ...
    if args.running_mode in [0, 2]:
        ...
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            failed = 0
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="CashThread", leave=True
            ):
                try:
                    future.result()  # 获取结果并处理可能的异常
                    completed += 1
