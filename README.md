...
- 🧊 `scripts/prospective_evaluate.py`：先校验冻结源码哈希，只评估2026186之后真实到达的开奖，并为明确期号生成不可覆盖的候选封存清单。
...
`results/` 默认不提交。开奖前候选封存清单位于
`reports/prospective_manifests/<期号>.json`并进入版本控制。浮动奖封顶情景不是逐期实际兑付；规则版本和替代输入见
...
6. **旧高级脚本能直接当科学回测吗？**
   ...
   使用 `scripts/prospective_evaluate.py --next-issue <官方期号>`，它按期号升序并保证预测第t期只读取t之前数据。省略参数时只读取冻结配置中的明确期号，不用“最新期号+1”推断。
7. **为什么不能重新运行旧final holdout来选择参数？**
   ...
   2026187一个新增期，不能得出统计显著性结论。2026187只是首个冻结后样本外观察，具体票面未在开奖前公开封存；2026188是首个提交完整预先封存候选集的期号。
