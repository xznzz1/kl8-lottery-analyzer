# 开发假设与约束

1. **仅支持快乐 8**  
   当前代码与配置均围绕快乐 8（`kl8`）玩法设计，其他彩票类型暂不维护。
2. **Python 3.11 环境**  
   默认在名为 `python311` 的 Conda 环境或等效的 Python 3.11 虚拟环境中执行，确保依赖一致。
3. **下载数据不入库**
   完整历史数据保存于 `data_cache/kl8/data.csv` 并默认忽略；正式分析需自行下载最新数据并以下载元信息中的 SHA-256 固定输入快照。
4. **外部网络访问受限**  
   `src/data_fetcher.py` 只允许访问白名单域名（500.com / 917500），若不可达则下载失败。
5. **分析脚本以 CLI 形式运行**  
   `src/analysis` 下脚本保持命令行入口，导入时如需自定义参数需显式传入或使用子进程调用。
6. **Dirichlet 与规则参数配置**  
   先验强度、方差惩罚与频繁项集阈值默认使用 `config.analysis` 中的值，可通过 CLI 覆盖，但需评估样本量与缓存命中率。
7. **Copula 采样样本量前提**  
   `src/analysis/copula_sampler.py` 估计 80 维相关结构，推荐 `limit_line ≥ analysis.copula.min_draws`（默认 180）；不足时会跳过并记录日志。
8. **图嵌入缓存一致性**  
   期望在分析前执行 `make train-graph`（或直接运行 `scripts/train_graph_embeddings.py`）生成最新嵌入；若缓存缺失，特征通道将自动退化为零向量。
9. **互信息惩罚权重可调**  
   互信息矩阵基于当前窗口估计，如需缩短窗口或切换玩法，应同步调整 `analysis.graph_embedding.weight`，避免惩罚过大或过小。
10. **科学评估不预设可预测性**
   开奖按随机过程解释；策略只在时间顺序样本外与多seed均匀随机基线比较，负结果必须如实报告。
11. **固定预算与出票方式**
   每期恰好两注、每注2元。`disjoint` 强制两注无重叠；`independent` 分别优化并允许重叠，确定性策略可能生成相同两注。
12. **反过拟合切分**
   最后20%原为一次性final holdout。当前326期结果已经查看并永久冻结，不得再用于选择策略或重新调参；窗口、衰减与hybrid权重只在此前的滚动验证区间、预注册小网格上选择，新主张需使用未来数据或新的未接触holdout。
13. **奖金版本与情景**
   第2025350期起切换新规则。名义固定奖可能受限赔；浮动奖缺失时明确失败。默认复现命令必须显式选择封顶情景，且报告不得称为历史实际兑付。
14. **旧高级功能的科学边界**
   旧深度特征、规则缓存和全样本图嵌入因时间方向或训练截止点不可审计而排除。科学高级基线只使用每个预测时点之前重算的特征，并禁用图嵌入缓存。
15. **固定存储边界**
   本次Windows审计的实际项目根为`D:\lottery\kl8-lottery-analyzer`，自动执行不得在C盘创建仓库副本、虚拟环境、缓存或结果。代码实现的可移植约束是：输出只允许落入当前仓库的`data_cache/`、`results/`、`reports/`，临时与工具缓存使用仓库父目录的`.tmp/`、`.cache/`。Makefile按自身位置解析仓库和`.venv`，不宣称把任意检出强制到D盘。
16. **覆盖率边界**
   覆盖率门槛针对当前轻量下载、配置、可复用分析组件和科学评估路径；旧深度训练/预测入口`common_legacy.py`、`modeling.py`、`pipeline.py`及`analysis/kl8_*`、`shared_cash.py`明确排除。这些模块不在默认下载、统计或科学回测调用链中，恢复维护前必须另补测试。
17. **永久冻结与前瞻起点**
   原科学报告、截止2026186的1630期数据快照及326期final holdout已经查看并永久冻结。前瞻模式只接受期号大于2026186的开奖记录，不调用时间切分或验证调参，也不读取或写入`results/scientific/`。
18. **完整冻结参数**
   顶层参数固定为窗口120、衰减0.99、hybrid权重(0.25, 0.50, 0.25)及20个seed。`repository_advanced`另有窗口40/160、衰减0.97、特征权重、Dirichlet、PCA及科学计算库版本，均在`config/scientific_freeze.json`显式记录并由运行时校验；NumPy、Pandas与scikit-learn在`requirements.txt`精确锁定。这只是把既有实现冻结，不是重新调参。
19. **单期证据限制**
   当前只有2026187一个新增时间簇。20个随机seed是同期开奖条件下的Monte Carlo重复，不能当成20个独立期开奖；因此前瞻摘要不运行p值、多重检验或排名，单期区间字段留空，不能声称优于随机或与随机等效。
20. **候选票面的语义**
   `next_issue_candidates.csv`只包含五个冻结确定性策略。`independent`表示两注分别优化且允许重叠，并非统计独立；在确定性排序下两注通常相同。候选是可复现代码输出，不是投注建议、预测概率或策略有效性证据。
21. **源码冻结边界**
   `source_manifest`严格覆盖九个策略实现、统计、前瞻入口和配置文件，文本规范化只把CRLF/CR转换为LF，不裁剪空白或末尾换行。任一哈希变化都属于新策略版本，现有前瞻链必须停止；`implementation_baseline_commit`不能替代逐文件校验。
22. **期号与证据等级**
   下一候选期号由CLI或冻结配置明确提供，不从整数加一推断。2026187是首个冻结后样本外观察，但具体票面没有开奖前版本化封存；2026188在`reports/prospective_manifests/2026188.json`提交后才是首个具有完整预先封存候选集的期号。
23. **候选manifest不可变**
   每期manifest内嵌规范化CSV的完整100行内容并记录历史、冻结配置、源码与候选哈希。首次创建后禁止覆盖；重算不一致和当前候选CSV篡改均fail-closed，新一期只能新增文件。
