...
| `src.scientific.prospective` | `load_scientific_freeze(path)` | 版本化JSON路径 | `ScientificFreeze` | 在读取开奖前校验六个冻结源码/配置的规范化SHA-256与清单指纹，再校验截止期、参数、seed、奖金情景和快照元信息；漂移要求新策略版本。 |
| `src.scientific.prospective` | `run_prospective_evaluation(paths, bootstrap_samples=1000, next_issue=None)` | 固定边界路径、描述区间次数、显式期号或冻结配置明确值 | `ProspectiveRunResult` | 只评估2026186之后数据，幂等核对已有记录，生成摘要/候选/报告及不可覆盖的逐期候选manifest；不推断下一期号、不执行切分或调参。 |
