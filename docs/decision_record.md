...
- `source_manifest`对九个策略实现、统计汇总、前瞻入口和配置文件采用严格UTF-8读取、仅规范化CRLF/CR为LF后计算SHA-256，并对有序路径—哈希映射再计算指纹。CLI可在导入策略依赖前核对自身磁盘文本；核心API在读取数据前复检。自检入口已开始执行是明确的bootstrap边界，但任何普通源码漂移仍会要求建立新策略版本，不能只依赖基线commit字符串。
- 2026188原封存版本保留在Git提交`c1f2887`。本轮因明确扩展source manifest形成新冻结契约，受控重建同路径manifest；正常CLI没有覆盖开关。迁移以原100行`strategy/play/ticket_mode/ticket1/ticket2`投影SHA-256 `5111264f6f0fd8d1c8d41a1639d2224d44100f6eba2e3e0dd5773141ae19e900`为固定回归契约，逐字段差异必须为0。
- 每个候选目标期在`reports/prospective_manifests/<期号>.json`建立版本化封存，内嵌完整100行规范化候选及历史、冻结配置、源码清单、候选CSV哈希。已有期号manifest从不覆盖；同一期重算必须逐字段完全一致，当前候选CSV在跨期覆盖前也必须先通过其原manifest验封。
- 2026187仅称为“首个冻结后样本外观察”：它按冻结参数重放，但具体票面未在开奖前公开封存。2026188的manifest提交后，才称为“首个具有完整预先封存候选集的期号”。这一区分不改变任何统计结果。
- 下一期号来自显式`--next-issue`或冻结配置中的明确值（当前为2026188）；永不使用`latest_issue + 1`，避免跨年生成不存在的期号。新期开奖到达后必须提供新的官方目标期号。
