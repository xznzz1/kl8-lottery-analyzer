...
def build_report(
    ...
    *,
    ...
    brier_delta = _ordinary_mean_interval(
    ...
    log_delta = _ordinary_mean_interval(
...
- 动态 Beta-Bernoulli 在 `{len(result.outer_indices)}` 个外层开奖期上的平均 Brier score 为 `{dynamic_brier[0]:.9f}`，相对公平随机概率基线的差值为 `{brier_delta[0]:+.9f}`；逐期开奖差值的普通均值标准误近似区间为 `[{brier_delta[2]:+.9f}, {brier_delta[3]:+.9f}]`，未校正潜在时间相关性，仅作描述。Brier 和 log loss 都是越低越好，因此这里的正差表示动态模型在这段已查看历史上略差。
...
所有均值和差值都以开奖期为基本观察单位。没有把同一期80个号码、20个随机seed或多个k值当成独立样本，也没有报告机会性p值。
...
固定 `decay` 时，记折扣出现次数为 `S_i = Σ decay^age × y_i`，所有号码共享折扣总权重 `W = Σ decay^age`。于是：

```text
posterior_mean_i = (prior_strength × 0.25 + S_i) / (prior_strength + W)
```

它是 `S_i` 的正仿射变换；相同 `decay` 的 `exponential_frequency` 得分 `S_i/W` 也是 `S_i` 的正比例变换。因此两者完整80位排名完全一致。`prior_strength` 只影响概率向0.25的收缩程度、后验方差、可信区间、Brier和log loss，不影响号码排序。3×3网格虽然有9种概率设定，但只有3种不同排名，对应3个 `decay`。

所以第一阶段动态模型的新增价值是概率表达、校准评估和不确定性量化，不是新的号码排序机制。比较表中的 `exponential_frequency` 使用冻结基线参数 `decay=0.99`，而 `dynamic_bayesian` 的 decay 由内层验证逐期选择；只有两者 decay 相同时才要求排名逐位一致。
...
- **区间是描述性的。** 报告中的差值区间是逐期开奖差值的普通均值标准误近似，未校正潜在时间相关性，不是确认性置信区间或显著性检验，也未做多重比较推断。
