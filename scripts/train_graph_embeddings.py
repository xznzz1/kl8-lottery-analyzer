...
from src.analysis.shared_download import ensure_data_available  # noqa: E402
...
def _random_walk(
    adjacency: np.ndarray, start: int, walk_length: int, rng: random.Random
) -> List[int]:
    node = start
    walk = [node]
...
class SkipGramModel(nn.Module):
    ...
    def forward(
        ...
        negative_loss = (
            -torch.log(torch.sigmoid(-negative_score) + 1e-9).sum(dim=1).mean()
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--embedding-dim",
        type=int,
        default=int(GRAPH_EMBED_CONFIG.get("embedding_dim", 32)),
    )
    parser.add_argument("--walk-length", type=int, default=16, help="随机游走长度")
    parser.add_argument(
        "--num-walks", type=int, default=20, help="每个节点随机游走次数"
    )
    ...
    parser.add_argument(
        "--negative-samples", type=int, default=5, help="每个正样本的负样本数"
    )
    parser.add_argument("--learning-rate", type=float, default=0.025, help="学习率")
    parser.add_argument(
        "--decay",
        type=float,
        default=float(GRAPH_EMBED_CONFIG.get("decay", 0.92)),
        help="历史期权重衰减",
    )
    parser.add_argument(
        "--max-samples", type=int, default=0, help="限制使用的历史期数（0 表示不限）"
    )
    parser.add_argument(
        "--device", default="auto", help="计算设备（auto/cpu/cuda/mps/xpu 等）"
    )
    parser.add_argument("--seed", type=int, default=42, help="随机种子")
    parser.add_argument(
        "--output",
        default=GRAPH_EMBED_CONFIG.get("cache_file"),
        help="嵌入输出路径（必须位于仓库 data_cache 内）",
    )
...
def main() -> None:
    ...
    draws = _load_draws(
        data_path, limit=args.max_samples if args.max_samples > 0 else None
    )
